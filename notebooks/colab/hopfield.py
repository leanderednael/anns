...
class HopfieldNetwork:
    ...
    def _set_weights(self, targets, alg):
        ...
        if alg == "Hebb":
            ...
            rho = (
                0  # Centering the targets using np.mean(targets) (or only over axis=1)
            )
            W = np.dot(
                targets - rho, targets.T - rho
            )  # Sum of outer products of columns of targets
            ...
            Dphi = np.concatenate(
                [np.exp(h) * np.ones(k), np.exp(-tau * h) * np.ones(self.D - k)]
            )
            ...
            raise ValueError(
                "Unknown learning algorithm, supported algorithms are 'Hebb' and 'LSSM'."
            )
    ...
    def _step(self, s, sync):
        if sync:
            ...
            for d in self.rng.permutation(self.D):
                # Update s - use only the d-th component of the bias
                s[:, d] = self.tf(
                    s @ self.W[:, d] + self.b[0, d]    # Vectorized version of W[d,:] s + b[d]
                )
        return s
    ...
    def energy(self, s):
        ...
        e = (
            -0.5 * np.sum((s @ self.W) * s, axis=1, keepdims=True) - s @ self.b.T
        )  # Vectorized version of - 1/2 s^T W s - s^T b
        e += np.sum(
            self.int_inv_tf(s), axis=1, keepdims=True
        )  # Vectorized version of sum_{i=1}^D int_{0}^{s_i} g^{-1}(z) dz (see energy equation on page 1407 of [LSSM] or lecture slide 19)
        return np.squeeze(e)
